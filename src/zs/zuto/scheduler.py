...
class ZutoScheduler:
    def __init__(self, path : str):
        ...
        self.observer.schedule(self.event_handler, path, recursive=True)  # Now using absolute path
        self.jobs = {}
        ...
        for file in os.listdir(path):
            if file.endswith(".yml") or file.endswith(".yaml"):
                self.add_job(file, ZutoJob.from_file(os.path.join(path, file), self.ctx))
            elif file.endswith(".py"):
                mod = import_file(file, "zs.zuto.funcs")
                eligibles = [
                    name for name in dir(mod.Cmds) if not name.startswith('_')
                ]
                for name in eligibles:
                    if name in self.ctx.funcMaps:
                        print(f"Warning: Function {name} already exists in ctx.funcMaps")
                        continue
